...
This callback function requires 2 parameter to specify whether
the promise is successful/ failure.
NOw, those 2 parameter will work as return statements and to store
the data into object and define the state of promise.

let parameter is for fulfilled
...
p.then(data=>{
    console.log("I am then method - Promise Fulfilled");
    console.log(data);
})
...
let shivani=new Promise((resolve,reject)=>{
    ...
    if(isPlaced){
        ...
        reject("shivani not got placced")
    }
});
...
let vikash= new Promise((resolve,reject)=>{
    let isPlaced = true;

    if(isPlaced){
        resolve("vikash got placed")
    }
    else{
        reject("vikash not placed yet")
    }
});
...
//? any:
//If any one of the promise got fullfileed then this method will work
//~ Promise.any([promise1,promise2,....]);
//Promise.any method will result  a promise as output so we have to consume it by using then /catch/ finally

// the any function a promise thst is fulllfileled by the given promise to be fullfilled.

// or rejected with an aggregateError containing an array of rejection resonsif all of the given promise are rejected.
// It resolves all elements of the passed iterable to promise as it runs this algorithm.

// @param values - An array or iterable of Promise.
...
// console.log(Promis.allSettled(['shivni','utkarsh', 'vikas', 'rishi']));


//? race():
// It will result the first fullfilled promise of all the promise .
// Creates a Promise that is resolved or rejected when any of the provided Promise are resolve or rejected .
// @param values - An array of Promise.
// @returns - A new Promise .
...
let rishi = new Promise((resolve, reject) => {
 ...
 console.log(Promise.race([rishi,Utkarsh,shivani,vikas]));
 
Promise.race([rishi,Utkarsh,shivani, vikas,]).then(data =>{
    console.log(data);
